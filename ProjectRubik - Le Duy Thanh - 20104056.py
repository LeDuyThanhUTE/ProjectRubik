...
stickers = {
    'main': [
        ...
        [150, 220], [250, 220], [350, 220],
        [150, 320], [250, 320], [350, 320]
    ],
    'current': [
        [20, 20], [54, 20], [88, 20],
        [20, 54], [54, 54], [88, 54],
...
textPoints = {
    'up': [['U', 242, 202], ['Y', (2, 255, 255), 260, 208]],
    'right': [['R', 380, 354], ['G', (0, 255, 0), 398, 360]],
    'front': [['F', 242, 354], ['R', (0, 0, 255), 260, 360]],
    'down': [['D', 242, 508], ['D', (255, 255, 92), 260, 514]],
...
def rotate(side):
    ...
    if side == 'front':
        left[2], left[5], left[8], up[6], up[7], up[8], right[0], right[3], right[6], down[0], down[1], down[
            2] = down[0], down[1], down[2], left[8], left[5], left[2], up[6], up[7], up[8], right[6], right[
            3], right[0]
    elif side == 'up':
        left[0], left[1], left[2], back[0], back[1], back[2], right[0], right[1], right[2], front[0], front[1], front[
            2] = front[0], front[1], front[2], left[0], left[1], left[2], back[0], back[1], back[2], right[0], right[
            1], right[2]
    elif side == 'down':
        ...
        left[0], left[3], left[6], up[0], up[1], up[2], right[2], right[5], right[8], down[6], down[7], down[
            8] = up[2], up[1], up[0], right[2], right[5], right[8], down[8], down[7], down[6], left[0], left[
            3], left[6]
    elif side == 'left':
        front[0], front[3], front[6], down[0], down[3], down[6], back[2], back[5], back[8], up[0], up[3], up[
            6] = up[0], up[3], up[6], front[0], front[3], front[6], down[6], down[3], down[0], back[8], back[
            5], back[2]
    elif side == 'right':
        front[2], front[5], front[8], down[2], down[5], down[8], back[0], back[3], back[6], up[2], up[5], up[
            8] = down[2], down[5], down[8], back[6], back[3], back[0], up[8], up[5], up[2], front[2], front[
            5], front[8]
...
def revrotate(side):
    ...
    if side == 'front':
        left[2], left[5], left[8], up[6], up[7], up[8], right[0], right[3], right[6], down[0], down[1], down[
            2] = up[8], up[7], up[6], right[0], right[3], right[6], down[2], down[1], down[0], left[2], left[
            5], left[8]
    elif side == 'up':
        left[0], left[1], left[2], back[0], back[1], back[2], right[0], right[1], right[2], front[0], front[1], front[
        ...
        left[6], left[7], left[8], back[6], back[7], back[8], right[6], right[7], right[8], front[6], front[7], front[
            8] = front[6], front[7], front[8], left[6], left[7], left[8], back[6], back[7], back[8], right[6], right[
            7], right[8]
    elif side == 'back':
        left[0], left[3], left[6], up[0], up[1], up[2], right[2], right[5], right[8], down[6], down[7], down[
            8] = down[6],down[7], down[8], left[6], left[3], left[0], up[0], up[1], up[2], right[8], right[
            5], right[2]
    elif side == 'left':
        front[0], front[3], front[6], down[0], down[3], down[6], back[2], back[5], back[8], up[0], up[3], up[
            6] = down[0], down[3], down[6], back[8], back[5], back[2], up[0], up[3], up[6], front[0], front[
            3], front[6]
    elif side == 'right':
        front[2], front[5], front[8], down[2], down[5], down[8], back[0], back[3], back[6], up[2], up[5], up[
            8] = up[2], up[5], up[8], front[2], front[5], front[8], down[8], down[5], down[2], back[6], back[
            3], back[0]
...
def solve(state):
    ...
    for i in state:
        for j in state[i]:
            raw += sign_conv[j]
    print(raw)
    print("answer:", utils.solve(raw,'Kociemba'))
    solution = utils.solve(raw,'Kociemba')
...
def color_detect(h, s, v):
    print(h,s,v)
    if h>120 and v < 230 and s > 130 and s <210:
        return 'red'
    elif h < 20 and s >= 45:
        return 'orange'
    elif h >= 25 and h < 43:
        return 'yellow'
    elif h >= 40 and h <= 90 and s > 60 and v > 150:
        return 'green'
    elif h <= 110 and h > 85 and s >50:
        return 'blue'
    elif h <= 100 and s < 45 and v < 255 and v>100:
        return 'white'
...
if __name__ == '__main__':
    ...
    Mbox('Hướng dẫn', 'Sử dụng nút: U,F,L,R,B,D để lấy màu các mặt Rubik,bấm H để hiện lại hướng dẫn, bấm Esc để thoát',0)
    while True:
        hsv = []
        current_state = []
        ret, img = cap.read()
        ...
        draw_stickers(preview, stickers, 'current')
        ...
        for x, y in stickers['current']:
            ...
            cv2.rectangle(preview, (x, y), (x + 30, y + 30), color[color_name], -1)
            a += 1
        ...
        if k == 27:
            ...
            if len(set(check_state)) == 6:
                try:
                    ...
                    Mbox('Lỗi', 'Lỗi, không tìm thấy giải pháp, hãy kiểm tra lại', 0)
                    print("Lỗi, không tìm thấy giải pháp, hãy kiểm tra lại")
            else:
                Mbox('Lỗi', 'Có thể bạn chưa scan toàn bộ các mặt, các mặt còn lại cần scan: {}'.format(6 - len(set(check_state))), 0)
                print("Có thể bạn chưa scan toàn bộ các mặt")
                print("các mặt còn lại cần scan:", 6 - len(set(check_state)))
